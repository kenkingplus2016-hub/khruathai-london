...
let i = 100; // start at 100 to avoid ID collisions if any exist

html = html.replace(/(<a href="mailto:info@zabsiam\.co\.uk\?subject=Customize:[^>]*>Enquire to Customize Menu<br><\/a>)/g, (match) => {
    i++;
    const btn = `<button onclick="document.getElementById('allergen-modal-idx${i}').style.display='flex'" class="add-to-cart-btn" style="text-align: center; display: block; margin-top: 10px; margin-bottom: 10px; line-height: 1.4; background-color: transparent; border: 1px solid var(--color-gold); color: var(--color-gold);">View Allergens</button>`;
    
    const modal = `
<div id="allergen-modal-idx${i}" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); z-index: 9999; justify-content: center; align-items: center; text-align: left;">
    <div style="position: relative; background: #111; max-width: 800px; width: 90%; max-height: 90vh; overflow-y: auto; padding: 30px; border: 1px solid var(--color-gold); border-radius: 8px;">
        <button onclick="document.getElementById('allergen-modal-idx${i}').style.display='none'" style="position: absolute; right: 15px; top: 15px; background: transparent; border: none; color: var(--color-gold); font-size: 2rem; cursor: pointer;">&times;</button>
...
console.log(`Updated index.html banquets with View Allergens.`);
